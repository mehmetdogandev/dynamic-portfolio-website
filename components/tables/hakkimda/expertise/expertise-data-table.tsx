...
import { DeleteExpertiseDialog } from './delete-expertise-dialog'
import { DetailExpertiseDialog } from './detail-expertise-dialog'
...
export function ExpertiseDataTable() {
  const trpc = useTRPC()
  const queryClient = useQueryClient()
  const router = useRouter()
  const [deleteRow, setDeleteRow] = useState<AdminAboutExpertiseRow | null>(
    null
  )
  const [detailRow, setDetailRow] = useState<AdminAboutExpertiseRow | null>(
    null
  )
  ...
  const { orderedIds: scopeOrderedIds } = useReorderScope({
    ...
    queryFn: () =>
      queryClient.fetchQuery(
        trpc.aboutExpertise.listReorderScope.queryOptions()
      ),
  })

  const { data, isLoading, isError, error } = useQuery({
    ...trpc.aboutExpertise.list.queryOptions(listInput),
  })

  const rows = useMemo(
    () => (data?.data ?? []) as AdminAboutExpertiseRow[],
    [data?.data]
  )
  ...
  const { mutateAsync: reorderAsync } = useMutation(
    trpc.aboutExpertise.reorder.mutationOptions({
      onSuccess: async () => {
  ...
  const columns: ColumnDef<AdminAboutExpertiseRow>[] = useMemo(
    () => [
      {
        id: 'sort',
        header: 'Sıra',
      ...
      createIconActionColumn<AdminAboutExpertiseRow>((row) => {
        const actions = []
        if (canRead) {
          actions.push({
            icon: Eye,
        ...
        if (canUpdate) {
          actions.push({
            ...
            label: 'Düzenle',
            onClick: () =>
              router.push(adminHref(`/hakkimda/expertise/${row.original.id}`)),
          })
        }
        if (canDelete) {
          actions.push({
  ...
  const renderTableBody = useCallback(
    (table: ReactTable<AdminAboutExpertiseRow>) =>
      table
        .getRowModel()
        .rows.map((row) => (
          <SortableExpertiseTableRow
            key={row.id}
            row={row}
          ...
          toolbarAdd={
            canCreate ? (
              <Button
                ...
                size="sm"
                onClick={() =>
                  router.push(adminHref('/hakkimda/expertise/new'))
                }
              >
...
function SortableExpertiseTableRow({
  row,
  displayIndex,
  canReorder,
}: {
  row: Row<AdminAboutExpertiseRow>
  displayIndex: number
  canReorder: boolean
}) {
  const {
