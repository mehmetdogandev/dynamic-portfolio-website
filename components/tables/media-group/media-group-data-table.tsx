...
import { CreateMediaGroupDialog } from './create-media-group-dialog'
import { DeleteMediaGroupDialog } from './delete-media-group-dialog'
import { DetailMediaGroupDialog } from './detail-media-group-dialog'
import { UpdateMediaGroupDialog } from './update-media-group-dialog'

export type AdminMediaGroupRow = {
  id: string
  name: string
  description: string | null
  parentMediaGroupId: string | null
...
export function MediaGroupDataTable() {
  const trpc = useTRPC()
  ...
  const { data: canRead } = usePermission(SCOPES.MEDIA_GROUP, PERMISSIONS.READ)
  ...
  const [detailRow, setDetailRow] = useState<AdminMediaGroupRow | null>(null)
  const [editRow, setEditRow] = useState<AdminMediaGroupRow | null>(null)
  const [deleteRow, setDeleteRow] = useState<AdminMediaGroupRow | null>(null)
  ...
  const { orderedIds: scopeOrderedIds } = useReorderScope({
    ...
    queryKey: trpc.mediaGroup.listReorderScope.queryKey(),
    queryFn: () =>
      queryClient.fetchQuery(trpc.mediaGroup.listReorderScope.queryOptions()),
  })

  const { data, isLoading, isError, error } = useQuery({
    ...trpc.mediaGroup.list.queryOptions(listInput),
  })

  const rows = useMemo(
    () => (data?.data ?? []) as AdminMediaGroupRow[],
    [data?.data]
  )
  ...
  const { mutateAsync: reorderAsync } = useMutation(
    trpc.mediaGroup.reorder.mutationOptions({
      onSuccess: async () => {
  ...
  const columns: ColumnDef<AdminMediaGroupRow>[] = useMemo(
    () => [
      {
        id: 'sort',
        header: 'Sıra',
      ...
      },
      {
        ...
        meta: {
          ...
          cellClassName: 'text-muted-foreground max-w-[340px] truncate text-sm',
        },
        cell: ({ row }) => row.original.description ?? '—',
      },
      createIconActionColumn<AdminMediaGroupRow>((row) => {
        const actions = []
        if (canRead) {
          actions.push({
            icon: Eye,
  ...
  const renderTableBody = useCallback(
    (table: ReactTable<AdminMediaGroupRow>) =>
      table
        .getRowModel()
        .rows.map((row) => (
          <SortableMediaGroupTableRow
            key={row.id}
            row={row}
  ...
  return (
    <div className="space-y-4">
      ...
      <CreateMediaGroupDialog open={createOpen} onOpenChange={setCreateOpen} />
      {detailRow ? (
        <DetailMediaGroupDialog
          row={detailRow}
          open={true}
...
function SortableMediaGroupTableRow({
  row,
  displayIndex,
  canReorder,
}: {
  row: Row<AdminMediaGroupRow>
  displayIndex: number
  canReorder: boolean
}) {
  const {
