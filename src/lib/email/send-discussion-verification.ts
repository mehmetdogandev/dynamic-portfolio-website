...
export interface SendDiscussionVerificationParams {
  to: string;
  username: string;
  verifyUrl: string;
}

export async function sendDiscussionVerification({
  to,
  username,
  verifyUrl,
}: SendDiscussionVerificationParams): Promise<void> {
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff; border-radius:14px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:40px 40px 10px 40px;">
              <div style="font-size:24px; font-weight:700; color:#0f172a;">
                Yorumunuzu Onaylayın
              </div>
            </td>
          </tr>
          <tr>
            <td style="padding:10px 40px; font-size:16px; color:#334155; line-height:1.7;">
              Merhaba ${username},<br><br>
              Projede bıraktığınız yorumu yayınlamak için e-posta adresinizi doğrulamanız gerekiyor.
              Aşağıdaki butona tıklayarak yorumunuzu onaylayabilirsiniz.
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:20px 40px; font-size:14px; color:#475569; line-height:1.6;">
              ...
              <span style="word-break:break-all; color:#2d4a7c;">${verifyUrl}</span>
            </td>
          </tr>
          <tr>
  ...
  const info = await transporter.sendMail({
    from: fromAddress,
    to,
    subject: "Yorumunuzu Onaylayın – mehmetdogandev.com",
    html,
    text: `Yorumunuzu onaylamak için bu bağlantıya tıklayın: ${verifyUrl}`,
  });
